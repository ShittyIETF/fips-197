...
void testExpansions() {
  ...
  {
    CipherKey<256> key;
    for (int c = 0; c < ::Cipher::Cipher<256>::key_length; ++c) {
      for (int r = 0; r < 4; ++r) {
        key[c][r] = key256[4*c + r];
      }
    }

    printf("Expanding key: %s\n", hexaString<8>(static_cast<array<Vec4, 8>&>(key)).c_str());

    CipherKeyExpansion<256> exp = expandKey(key);

    printf("Expected: %s\n", expected256);
    printf("Actual:   %s\n", hexaString< ::Cipher::Cipher<256>::expansion_length >(
      static_cast< array<Vec4, ::Cipher::Cipher<256>::expansion_length >& >(exp)).c_str()
    );
  }
}
